...
export function DestinationAndDateStep({
  ...
  const [isDaterPickerOpen, setIsDaterPickerOpen] = useState(false)

  function openDatePicker() {
    return setIsDaterPickerOpen(true)
  }

  function closeDatePicker() {
    return setIsDaterPickerOpen(false)
  }

  const displayedDate = eventStartAndDates && eventStartAndDates.from && eventStartAndDates.to
   ? format(eventStartAndDates.from, "d' de 'LLL").concat(' ate ').concat(format(eventStartAndDates.to, "d' de 'LLL"))
    : null
  ...
  return (
    <div className="h-16 bg-zinc-900 px-4 rounded-xl flex items-center shadow-shape gap-3">
      <div className="flex items-center gap-2 flex-1">
        ...
        <input disabled={isGuestsInpoutOpen} 
        type="text" 
      ...
      <button onClick={openDatePicker} disabled={isGuestsInpoutOpen}
